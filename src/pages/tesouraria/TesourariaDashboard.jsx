...
export default function TesourariaDashboard() {
  ...
  return (
      <div className="animate-in fade-in duration-700">
        ...
        {/* FILTRO BAR */}
        <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-[2rem] p-3 mb-8 flex flex-col sm:flex-row items-center gap-3 shadow-sm">
          ...
          {modo === "mensal" && (
              <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
                <button
                    ...
                    className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-lg transition-colors text-slate-500"
                >
                  <ChevronLeft size={15} />
                </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          <CardInterativo titulo="Dízimos" valor={resumo.DIZIMO} cor="indigo" icon={<Wallet size={22} />} loading={loading} onClick={() => navegarParaFiltro("TODOS")} />
          <CardInterativo titulo="Ouro"    valor={resumo.OURO}   cor="yellow" icon={<Award  size={22} />} loading={loading} onClick={() => navegarParaFiltro("OURO")} />
          <CardInterativo titulo="Prata"   valor={resumo.PRATA}  cor="slate"  icon={<Award  size={22} />} loading={loading} onClick={() => navegarParaFiltro("PRATA")} />
          <CardInterativo titulo="Bronze"  valor={resumo.BRONZE} cor="orange" icon={<Award  size={22} />} loading={loading} onClick={() => navegarParaFiltro("BRONZE")} />
        </div>

        {/* GRÁFICO + LATERAL */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white dark:bg-slate-900/60 p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/60 dark:shadow-none border border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg shadow-indigo-500/25">
                  <TrendingUp size={18} />
                </div>
                <div>
                  <h3 className="font-black text-base uppercase italic dark:text-white tracking-tighter leading-none">
                    Fluxo por Categoria
                  </h3>
              ...
              {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dadosGrafico} margin={{ top: 20, right: 30, left: -20, bottom: 0 }}>
                      ...
                      />
                      <Bar dataKey="valor" radius={[14, 14, 6, 6]} barSize={44}>
          ...
          {/* Painel lateral com resumo do período */}
          <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white flex flex-col justify-between shadow-2xl shadow-indigo-500/30 relative overflow-hidden group">
              ...
              <div className="mt-7 space-y-0">
                {[
                  ...
                  { label: "Total",   valor: totalGeral,     dot: "bg-white"      },
                    ...
                    <div
                        key={item.label}
                        className={`flex items-center justify-between py-3 ${i < 2 ? "border-b border-indigo-500/40" : ""}`}
                    >
            ...
            <button
                ...
                className="relative z-10 mt-8 w-full py-4 bg-white text-indigo-600 rounded-2xl font-black uppercase tracking-widest text-[11px] hover:scale-[1.02] transition-transform active:scale-95 shadow-lg"
            >
              Ir para Relatórios
            </button>

            <TrendingUp size={160} className="absolute -right-8 -bottom-8 opacity-10 group-hover:scale-110 transition-transform duration-700" />
          </div>
        </div>
      </div>
  );
...
function CardInterativo({ titulo, valor, cor, icon, onClick, loading }) {
  ...
  return (
      <button
          onClick={onClick}
          className={`group relative p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 border-b-4 bg-white dark:bg-slate-900 shadow-lg transition-all duration-500 text-left flex flex-col justify-between h-48 overflow-hidden hover:-translate-y-2 hover:shadow-xl ${styles[cor]}`}
      >
        <div className="flex justify-between items-start relative z-10">
          <div className="p-3 bg-slate-50 dark:bg-white/5 rounded-2xl group-hover:bg-current group-hover:text-white transition-all duration-500">
            {icon}
          </div>
          <ArrowRight size={18} className="opacity-0 group-hover:opacity-100 -translate-x-3 group-hover:translate-x-0 transition-all duration-300" />
        </div>

        <div className="relative z-10">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 mb-1">{titulo}</p>
          {loading ? (
              <div className="h-7 w-28 bg-slate-200 dark:bg-slate-700 rounded-lg animate-pulse mt-1" />
          ) : (
        ...
        <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-current opacity-[0.04] group-hover:opacity-[0.09] rounded-full blur-xl transition-opacity duration-500" />
      </button>
  );
}