...
const getCSS = (isDark) => `
  ...
  * { box-sizing: border-box; }
  ...
  .rm-month-btn:hover { background:rgba(200,16,46,.12); color:#C8102E; }
  .rm-month-label {
    font-family:'Cinzel',serif; font-size:10px; font-weight:700; letter-spacing:.16em;
    text-transform:uppercase; color:${isDark ? "#F5F0E8" : "#1A0A0D"}; min-width:110px; text-align:center;
  }

  .rm-year-select {
    background:${isDark ? "rgba(200,16,46,.08)" : "rgba(200,16,46,.04)"};
    border:1px solid ${isDark ? "rgba(200,16,46,.25)" : "rgba(200,16,46,.14)"};
  ...
  .rm-year-select option { background:${isDark ? "#1A0A0D" : "#fff"}; color:${isDark ? "#F5F0E8" : "#1A0A0D"}; }
  .rm-year-select:focus { border-color:#C8102E; box-shadow:0 0 0 3px rgba(200,16,46,.15); }
  ...
  .rm-search:focus { border-color:#C8102E; box-shadow:0 0 0 3px rgba(200,16,46,.15); }
  .rm-search::placeholder { color:${isDark ? "rgba(245,240,232,.3)" : "rgba(26,10,13,.3)"}; }
  ...
  .rm-table thead tr { background: ${isDark ? "rgba(200,16,46,.08)" : "rgba(200,16,46,.04)"}; }
  .rm-table th {
    padding:14px 16px; font-family:'Cinzel',serif;
    font-size:9px; font-weight:700; letter-spacing:.18em;
    text-transform:uppercase; color:${isDark ? "rgba(245,240,232,.4)" : "rgba(26,10,13,.4)"}; text-align:left; white-space:nowrap;
  }
  .rm-table th:last-child { text-align:right; }
  .rm-table td { padding:13px 16px; border-top:1px solid ${isDark ? "rgba(200,16,46,.1)" : "rgba(200,16,46,.07)"}; }
  @media (min-width:640px) { .rm-table th, .rm-table td { padding:16px 22px; } }
  .rm-table tbody tr { transition:background .15s; }
  .rm-table tbody tr:hover { background:${isDark ? "rgba(200,16,46,.07)" : "rgba(200,16,46,.04)"}; }
  ...
  .rm-date-pill {
    ...
    color:${isDark ? "rgba(245,240,232,.45)" : "rgba(26,10,13,.5)"};
  }
  ...
  .rm-header-row { display:flex; flex-direction:column; gap:14px; margin-bottom:22px; }
  @media (min-width:520px) { .rm-header-row { flex-direction:row; align-items:flex-end; justify-content:space-between; } }

  .rm-skel { animation: pulse 1.5s ease infinite; }
  .rm-skel-block { background:${isDark ? "rgba(200,16,46,.1)" : "rgba(200,16,46,.08)"}; border-radius:10px; }
`;
...
export default function TesourariaRegistrosMensal({ isDark = false }) {
  ...
  const textSecondary = isDark ? "rgba(245,240,232,.45)" : "rgba(26,10,13,.4)";
      ...
      <>
        ...
        <div className="rm-root" style={{ maxWidth:1100, margin:"0 auto", padding:"16px 4px" }}>
          ...
          <div className="rm-controls">
            ...
            <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:6, fontFamily:"'Cinzel',serif", fontSize:9, letterSpacing:".16em", textTransform:"uppercase", color:textSecondary }}>
              ...
              <table className="rm-table">
                ...
                {filtrados.length === 0 ? (
                    <tr><td colSpan={6} style={{ padding:"56px 0", textAlign:"center" }}>
                      ...
                      <p style={{ fontFamily:"'Cinzel',serif", fontSize:9, letterSpacing:".2em", color:textSecondary, textTransform:"uppercase" }}>Nenhum registro no período</p>
                    </td></tr>
                ) : filtrados.map(r => (
                    <tr key={r.id}>
                      <td style={{ fontFamily:"'Cinzel',serif", fontSize:10, color:textSecondary, letterSpacing:".08em" }}>
                        #{r.id.toString().padStart(4,"0")}
                      </td>
                      <td>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          <div className="rm-avatar">{r.membroNome?.charAt(0) || "?"}</div>
                          <span style={{ fontFamily:"'EB Garamond',serif", fontSize:15, color:textPrimary, fontWeight:500 }}>
                          {r.membroNome || "Anônimo"}
                        </span>
                        </div>
                      </td>
                      <td style={{ fontFamily:"'Cinzel',serif", fontSize:12, fontWeight:700, color:C.blue, whiteSpace:"nowrap" }}>
                        {r.valorDizimo ? `R$ ${fmt(r.valorDizimo)}` : <span style={{ color:textSecondary }}>–</span>}
                      </td>
                      <td style={{ fontFamily:"'Cinzel',serif", fontSize:12, fontWeight:700, color:C.yellowDark, whiteSpace:"nowrap" }}>
                        {r.valorOferta ? `R$ ${fmt(r.valorOferta)}` : <span style={{ color:textSecondary }}>–</span>}
                      </td>
...
function RmBadge({ tipo }) {
  ...
  return (
      <span style={{ padding:"4px 12px", borderRadius:99, border:`1px solid ${s.border}`, fontFamily:"'Cinzel',serif", fontSize:"8.5px", fontWeight:700, letterSpacing:".16em", textTransform:"uppercase", whiteSpace:"nowrap", color:s.color, background:s.bg }}>
