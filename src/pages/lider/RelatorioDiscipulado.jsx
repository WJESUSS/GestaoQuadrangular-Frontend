...
const COLUNAS = [
  ...
  { campo: "domingoNoite",  label: "Dom. Noite",  emoji: "🕊️" },
];
...
export default function RelatorioDiscipulado({ isDark = false }) {
  ...
  // FIX: ref para acessar presencas sem acionar re-render desnecessário
  const presencasRef  = useRef([]);
  // FIX: ref para acessar membros sem usar setter aninhado
  const membrosRef    = useRef([]);

  // Mantém presencasRef sempre sincronizado com o estado
  useEffect(() => {
    presencasRef.current = presencas;
  }, [presencas]);

  // Mantém membrosRef sempre sincronizado com o estado
  useEffect(() => {
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      // FIX: usa variável local diretamente, sem setter aninhado
      ...
      if (draft?.presencas) {
        ...
        const merged = [...filtradas, ...novos];
        setPresencas(merged);
        presencasRef.current = merged;
        ...
        const iniciais = inicializarPresencas(lista);
        setPresencas(iniciais);
        presencasRef.current = iniciais;
        setFim(semana.fim);
      }
  ...
  const handleInicioChange = useCallback(async (novoInicio) => {
    ...
    if (draft?.presencas) {
      ...
      presencasRef.current = draft.presencas;
      ...
      // FIX: usa membrosRef.current diretamente, sem setter aninhado
      const iniciais = inicializarPresencas(membrosRef.current);
      setPresencas(iniciais);
      presencasRef.current = iniciais;
    }
    carregouRef.current = true;
  ...
  // FIX: não usa mais setPresencas(prev => { ...; return prev }) para apenas agendar save
  const handleFimChange = useCallback((novoFim) => {
    setFim(novoFim);
    fimRef.current = novoFim;
    // lê presencas via ref, sem forçar re-render
    agendarSave(presencasRef.current, novoFim);
  }, [agendarSave]);
  ...
  const entrarModoEdicao = useCallback(async (item, detalhePreCarregado) => {
    ...
    try {
      ...
      // FIX: usa membrosRef.current diretamente
      const merged = membrosRef.current.map(m => {
  ...
  }, []); // FIX: sem dependência em `membros` — usa ref
  ...
  const globalStyles = `
    ...
    * { box-sizing:border-box; }
    ...
    /* FIX: transform via classe CSS, não inline — evita reflow por leitura de layout */
    .ieq-presence-btn {
    ...
    .ieq-presence-btn.marcado { transform:scale(1.04); }
          ...
          <div className="ieq-card" style={{ padding:"28px 32px" }}>
            <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"space-between", gap:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button
                    className={`ieq-tab ${aba === "relatorio" ? "ieq-tab-active" : "ieq-tab-inactive"}`}
                    onClick={() => { setAba("relatorio"); setDetalheItem(null); }}
                >
                  <UserCheck size={13}/> RELATÓRIO
                </button>
                <button
                    className={`ieq-tab ${aba === "historico" ? "ieq-tab-active" : "ieq-tab-inactive"}`}
              ...
              <>
                ...
                {modoEdicao && (
                    <div className="ieq-edit-banner">
                      ...
                      {/* FIX: usa cancelarEdicao extraído, sem setter aninhado */}
                      <button onClick={cancelarEdicao} style={{ background:"none", border:`1px solid rgba(0,61,165,.3)`, borderRadius:8, padding:"8px 14px", cursor:"pointer", fontFamily:"'Cinzel',serif", fontSize:9, color:IEQ.blueLight }}>
                        CANCELAR
                      </button>
                    </div>
                )}
                ...
                <div className="ieq-card" style={{
                  ...
                  {/* Cabeçalho da tabela */}
                  <div style={{ display:"grid", gridTemplateColumns:"1fr repeat(5,60px)", padding:"14px 24px", gap:8, alignItems:"center", borderBottom:`1px solid ${isDark?"rgba(200,16,46,.1)":"rgba(200,16,46,.08)"}`, background:isDark?"rgba(200,16,46,.04)":"rgba(200,16,46,.03)" }}>
                    <span style={{ fontFamily:"'Cinzel',serif", fontSize:9, letterSpacing:".16em", color:ts }}>MEMBRO</span>
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8 }}>
                            {COLUNAS.map(({ campo, label, emoji }) => {
                              ...
                              return (
                                  <button
                                      key={campo}
                                      // FIX: transform via classe CSS, não inline
                                      className={`ieq-presence-btn${marcado ? " marcado" : ""}`}
