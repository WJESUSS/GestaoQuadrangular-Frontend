...
import {
  ...
  History, Edit3, AlertTriangle, ChevronLeft, Eye,
} from "lucide-react";
...
const HISTORICO_LIMITE = 3; // quantos cards exibir
...
// ─────────────────────────────────────────────────────────────────────────────
// ABA HISTÓRICO – exibe os últimos 3 sem paginação
// ─────────────────────────────────────────────────────────────────────────────
function AbaHistorico({ isDark, onVerDetalhe }) {
  ...
  // Apenas os últimos HISTORICO_LIMITE relatórios (API já retorna ordem desc)
  const slice = historico.slice(0, HISTORICO_LIMITE);

  return (
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>

        {/* Cabeçalho simples com contador */}
              ...
              <div key={item.id} style={{
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:"flex", gap:16, alignItems:"center" }}>
                    <span style={{ fontFamily:"'Cinzel',serif", fontSize:9, color:ts }}>
                    ...
                    <span style={{ fontFamily:"'Cinzel',serif", fontSize:9, fontWeight:700,
                      color: pct > 60 ? IEQ.yellow : IEQ.red }}>
                      {pct}% FREQ.
                    </span>
                  </div>
                  <div style={{ marginTop:10, height:3, borderRadius:99,
                    background: isDark ? "rgba(255,255,255,.06)" : "rgba(200,16,46,.08)", overflow:"hidden" }}>
                    <div style={{
                      height:"100%", width:`${pct}%`, borderRadius:99,
                      background: pct > 60 ? IEQ.yellow : `linear-gradient(90deg,${IEQ.red},${IEQ.blue})`,
...
export default function RelatorioDiscipulado({ isDark = false }) {
  ...
  const verificarExistente = useCallback(async (ini, fi) => {
    ...
    finally { setVerificandoExistente(false); }
  }, []);
  ...
  const carregarDados = useCallback(async () => {
    ...
    finally { setLoading(false); carregouRef.current = true; }
  ...
  const handleInicioChange = useCallback(async (novoInicio) => {
    carregouRef.current = false;
    setInicio(novoInicio); inicioRef.current = novoInicio;
  ...
  const entrarModoEdicao = useCallback(async (item, detalhePreCarregado) => {
    ...
    try {
      ...
      setPresencas(merged);
      carregouRef.current = true;
    } catch { setErro("Erro ao carregar dados para edição."); }
  }, [membros]);
        ...
        <div style={{ position:"relative", zIndex:1, maxWidth:800, margin:"0 auto", padding:"0 16px", display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {aba === "relatorio" && (
              <>
                        ...
                        <div key={m.id} className="ieq-member-block">
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8 }}>
                            {COLUNAS.map(({ campo, label, emoji }) => {
                              ...
                              return (
                                  <button key={campo} className="ieq-presence-btn" onClick={() => alterarPresenca(i, campo)} style={{ borderColor: marcado ? IEQ.red : (isDark ? "rgba(200,16,46,.15)" : "rgba(200,16,46,.12)"), background: marcado ? (isDark ? "rgba(200,16,46,.12)" : "rgba(200,16,46,.07)") : (isDark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)"), transform: marcado ? "scale(1.04)" : "scale(1)" }}>
                ...
                <button className="ieq-btn-submit" onClick={enviarRelatorio} disabled={enviando || loading || membros.length === 0 || (relatorioExistente && !modoEdicao)} style={{ background: modoEdicao ? `linear-gradient(135deg,${IEQ.blueDark},${IEQ.blue})` : `linear-gradient(135deg,${IEQ.redDark},${IEQ.red})`, boxShadow: modoEdicao ? "0 8px 24px rgba(0,61,165,.25)" : "0 8px 24px rgba(200,16,46,.25)" }}>
                  {enviando ? <><Loader2 size={17} className="spin-icon"/> PROCESSANDO...</> : modoEdicao ? <><Edit3 size={17}/> SALVAR ALTERAÇÕES</> : <><CheckCircle2 size={17}/> FINALIZAR RELATÓRIO DA SEMANA</>}
                </button>
              </>
          )}
